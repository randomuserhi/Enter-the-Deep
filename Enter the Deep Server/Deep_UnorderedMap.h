...
static Deep_Inline valueType* Deep_UnorderedMap_##keyTag##_To_##valueTag##_Insert(struct Deep_UnorderedMap(keyTag, valueTag)* unorderedMap, size_t hash, const keyType* key, int (*keyCompareFunc)(const void*, const void*, size_t)) \
{ \
	return _Deep_UnorderedMap_Insert(&unorderedMap->_unorderedMap, hash, key, keyCompareFunc); \
} \
static Deep_Inline void Deep_UnorderedMap_##keyTag##_To_##valueTag##_Erase(struct Deep_UnorderedMap(keyTag, valueTag)* unorderedMap, size_t hash, const keyType* key, int (*keyCompareFunc)(const void*, const void*, size_t)) \
{ \
	_Deep_UnorderedMap_Erase(&unorderedMap->_unorderedMap, hash, key, keyCompareFunc); \
}
...
static Deep_Inline void Deep_UnorderedMap_##keyTag##_To_##valueTag##_Create(struct Deep_UnorderedMap(keyTag, valueTag)* unorderedMap) \
{ \
	_Deep_UnorderedMap_Create(&unorderedMap->_unorderedMap, sizeof(keyType), Deep_AlignOf(keyType), sizeof(valueType), Deep_AlignOf(valueType)); \
}
...
static Deep_Inline void Deep_UnorderedMap_raw_To_raw_Create(struct Deep_UnorderedMap(raw, raw)* unorderedMap, size_t keyTypeSize, size_t keyTypeAlignment, size_t valueTypeSize, size_t valueTypeAlignment)
{
	_Deep_UnorderedMap_Create(&unorderedMap->_unorderedMap, keyTypeSize, keyTypeAlignment, valueTypeSize, valueTypeAlignment);
}
